...
The app calls `/api/**`, which `proxy.conf.json` forwards to `http://localhost:8080`. The Kotlin/Spring backend lives at **`../kayedaw-api`** (not `kayedaw-api-showcase`, as the README claims) and must be up or every screen shows "Serveur injoignable" — start it with `mvn spring-boot:run` (74 tests, all green).

Demo accounts, seeded on every backend start by `config/DonneesInitiales.kt` (H2 is in-memory, so they are recreated each run):
...
**Auth**: OAuth2 `client_credentials`. `JetonMeteoFranceService` caches the 1 h token, refreshes 5 min early, and serialises refreshes behind a `Mutex` with double-checked locking. The secret lives in **`kayedaw-api/config/application.yml`** — Spring Boot reads `./config/` automatically with priority over the classpath, it is gitignored, and (unlike `src/main/resources`) it is **not packaged into the JAR**. `METEOFRANCE_APPLICATION_ID` still overrides it for production. Absent both, the integration disables itself and Open-Meteo takes over — the app runs fine either way, which is exactly how the fallback gets tested.
